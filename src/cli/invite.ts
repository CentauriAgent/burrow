...
export async function inviteCommand(opts: {
  ...
  const encoding = keyPackageEvent.tags.find((t: string[]) => t[0] === 'encoding')?.[1] || 'hex';
  ...
  if (!decoded) { console.error('❌ Failed to decode KeyPackage'); pool.close(); process.exit(1); }
  const [mlsMessage] = decoded;
  if ((mlsMessage as any).wireformat !== 'mls_key_package') {
  ...
  const commitResult = await addMember(groupState, (mlsMessage as any).keyPackage);

  // Build and send Welcome via NIP-59 gift-wrap
  if (commitResult.welcome) {
    const welcomeBytes = encodeMlsMessage(commitResult.welcome as any);
  ...
  if (commitResult.newState) {
    const newEncodedState = serializeGroupState(commitResult.newState as any);
