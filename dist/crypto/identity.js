 ...
 */
export function loadIdentity(keyPath) {
    const path = keyPath || DEFAULT_KEY_PATH;
    if (!existsSync(path)) {
        throw new Error(`Secret key not found at ${path}. Run 'burrow init' to generate one.`);
    }
 ...
 */
export function generateIdentity(keyPath) {
    const path = keyPath || DEFAULT_KEY_PATH;
 ...
 */
export function getCredentialIdentity(identity) {
