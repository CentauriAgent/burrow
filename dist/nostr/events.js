 ...
 */
export function buildKeyPackageEvent(identity, keyPackageBase64, opts) {
    ...
    return createSignedEvent(MARMOT_KINDS.KEY_PACKAGE, keyPackageBase64, tags, identity.secretKey);
}
/**
 * Build a kind 445 Group Event (encrypted MLS message) per MIP-03.
 * Uses an ephemeral keypair for publishing.
 */
export function buildGroupEvent(nostrGroupIdHex, encryptedContent) {
    const tags = [
        ['h', nostrGroupIdHex],
    ];
    return createEphemeralSignedEvent(MARMOT_KINDS.GROUP_MESSAGE, encryptedContent, tags);
}
/**
 ...
 */
export function buildInnerChatMessage(senderPubkeyHex, text, replyToId) {
    const tags = [];
    if (replyToId) {
 ...
 */
export function buildWelcomeEvent(welcomeBase64, keyPackageEventId, relays) {
    return {
